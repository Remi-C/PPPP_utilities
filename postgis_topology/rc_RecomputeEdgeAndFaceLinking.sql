	...
	*/ 
	...
	BEGIN     	  
		...
		--for each node : 
				...
				-- if edge is going out of node, set next_right_edge 
		SELECT  * FROM rc_lib.rc_RecomputeEdgeLinking(topology_name , nodes_to_update) into updated_edges ; 
		SELECT  * FROM rc_lib.rc_RecomputeFaceLinking_fewedges(topology_name , updated_edges) into 
...
$BODY$  
	...
	*/ 
	...
	BEGIN     	  
		...
		SELECT * FROM rc_lib.rc_RecomputeFaceLinking_fewedges_onlyvalidface(topology_name  , edges_to_update)
		...
		SELECT * FROM  rc_lib.rc_RecomputeFaceLinking_fewedges_onlyflatface(topology_name
			,   _inserted_face
		...
		SELECT  * FROM rc_lib.rc_CorrectIsolatedNodes(topology_name,  NULL,  _faces_to_delete  )
		INTO  _updated_nodes; 
		...
		SELECT * FROM rc_lib.rc_DeleteUselessFace('bdtopo_topological') INTO _deleted_faces;
...
--SELECT rc_lib.rc_CorrectIsolatedNode('bdtopo_topological'::text, ARRAY[212], NULL::int[] ) ;
--SELECT rc_lib.rc_DeleteUselessFace('bdtopo_topological');
	...
	*/ 
	...
	BEGIN     	  
		...
		,real_faces AS ( --check wether the ring form a real face (i.e not a flat face)
			SELECT base_id, rc_lib.rc_IsRingFace(array_agg(edge_id)) as is_this_ring_a_real_face
			FROM rings
			GROUP BY base_id 
		)
...
$BODY$  
	...
	*/ 
	...
	BEGIN     	   
		...
		--FROM rc_lib.rc_RecomputeFaceLinking_fewedges_onlyvalidface('bdtopo_topological'  , ARRAY[405,406,407,411,410] )
		 )
		 ,faces_to_delete AS (
...
-- SELECT *
-- FROM  rc_lib.rc_CorrectIsolatedNode('bdtopo_topological', NULL, ARRAY[212])  ;