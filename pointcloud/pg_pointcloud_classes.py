...
def patch_string_buff_to_numpy(string_buf, mschema):
    """convert the output of psycopg of a pcpatch to a numpy array"""
    import binascii
    import numpy as np
    #convert the string to hex
    binary_string = binascii.unhexlify(string_buf)
    return np.frombuffer(binary_string, dtype=mschema.numpy_dtype, offset=1 + 4 + 4 + 4)
...
def patch_numpy_to_numpy_double(numpy_spec_dtype, mschema):
    ...
    import numpy as np
    #create result
    points_double = np.zeros((numpy_spec_dtype.size, mschema.ndims), dtype=np.float64)
...
def WKB_patch_to_numpy_double(binary_patch_text, mschema):
    """This function uses as input a binary representation of the patch
    and convert it to a 2D numpy double array
    """
    np_points = patch_string_buff_to_numpy(binary_patch_text, mschema)
...
def numpy_spec_to_patch(numpy_spec_array, mschema, pcid):
    ...
    values = [1, pcid, 0, numpy_spec_array.shape[0]]
...
def numpy_double_to_WKB_patch(numpy_double_patch, mschema, pcid):
    ...
    return numpy_spec_to_patch(numpy_spec, mschema, pcid)
...
def get_schema(pcid, connection_string):
    ...
    if str(pcid) in GD['rc']['schemas']:
        return GD['rc']['schemas'][str(pcid)]
        #if we get it, stop there

    #are we inside or outside the database (plpython or python)
    executing_in_postgres = executing_from_postgres()
    if executing_in_postgres == True:
        #use DBAPI to get the schema with given pcid
        query = """SELECT schema FROM pointcloud_formats WHERE pcid = %d""" % pcid
        schema_xml = plpy.execute(query, 1)
        plpy.notice(schema_xml)
        plpy.notice("toto")
    else:
        ...
        cur = conn.cursor()
        cur.execute("""SELECT schema FROM pointcloud_formats WHERE pcid = %s""", [pcid]); 
        schema_xml = cur.fetchone()[0]
        print schema_xml
        conn.commit()
        cur.close()
        conn.close()
    ...
    if executing_in_postgres == True:
        plpy.notice("writing in plpy.GD")        
        plpy.GD['rc']['schemas'][str(pcid)] = pc_schema
    else:
        GD['rc']['schemas'][str(pcid)] = pc_schema
    return pc_schema
...
def test_schema(schemas):
    ...
    np_points = patch_string_buff_to_numpy(patch_text, schema)
    #print np_points
    numpy_double = patch_numpy_to_numpy_double(np_points, schema)
    numpy_spec = numpy_double_to_numpy_spec(numpy_double, schema)
    patch = numpy_spec_to_patch(numpy_spec, 1)
    import difflib
