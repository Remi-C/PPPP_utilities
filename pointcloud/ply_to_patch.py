...
def writing_pgpatch_to_base(spec_array,schema,conn, cur, writing_query, schemas):
    ...
    #print spec_array
    #print "type" + str(type(spec_array))
    #print "dtype : " + str(spec_array.dtype)
    
    #shitty move, casting back to numpy double should not be necessary ! 
    numpy_double, schema = pgp.patch_numpy_to_numpy_double(np.array(spec_array), schema)
...
def making_pgpatch(numpy_spec_patch,connection_string,pcid,writing_query):
    ...
    for spec_array in numpy_spec_patch:
        writing_pgpatch_to_base(spec_array,schema,conn, cur, writing_query,schemas)
...
def ply_to_patch(ply_file_path,connection_string,pcid,writing_query):
    ...
    making_pgpatch(numpy_spec_patch,connection_string,pcid,writing_query)
