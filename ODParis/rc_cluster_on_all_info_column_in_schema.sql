...
DROP FUNCTION IF EXISTS rc_cluster_on_all_info_column(text);--remove the function before re-creating it : act as a security versus function-type change

CREATE OR REPLACE FUNCTION rc_cluster_on_all_info_column_in_schema(schema_name text) RETURNS boolean
AS $$
DECLARE
...
/*exemple use-case :*/
SELECT rc_cluster_on_all_info_column_in_schema('odparis_test');



DROP FUNCTION IF EXISTS rc_cluster_on_info_column(text,text);--remove the function before re-creating it : act as a security versus function-type change

CREATE OR REPLACE FUNCTION rc_cluster_on_info_column(schema_name text,table_name text) RETURNS boolean
AS $$
DECLARE
    row record;
    result boolean;
