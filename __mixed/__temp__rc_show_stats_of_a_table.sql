...
CREATE OR REPLACE FUNCTION rc_show_stats_of_a_table(schema_name text, table_name text, max_number_of_row integer DEFAULT 15) RETURNS refcursor AS $$
DECLARE
...
SELECT rc_show_stats_of_a_table('odparis','vol_bati',100) as the_cursor;
FETCH ALL FROM test_cursor;

END; --NOTE : DO omit the END; to see any result !
