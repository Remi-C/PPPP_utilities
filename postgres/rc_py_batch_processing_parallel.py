...
def batch_LOD_monoprocess((key_min,key_max,key_step)): 
    ...
    cur.execute("""SET search_path to vosges_2011, public;"""); 
    conn.commit(); 
    ...
    while i < key_max :
        #print i;
        #print i, i+key_step;
        execute_querry(cur, i,i+key_step)
        conn.commit() 
        i+=key_step ; 
    cur.close()
    conn.close()
...
def split_interval_into_smaller_interval(split_number,key_tot_start,key_tot_end, the_step):
    ...
    interval_to_process = [] ;
    for i,(proc) in enumerate(arange(1,split_number+1)):
        key_local_start = int(ceil(key_tot_start+i * key_range)) ;
        key_local_end = int(trunc(key_tot_start+(i+1) * key_range)); 
