...
BEGIN
	...
	RAISE NOTICE 'number of column :  %',number_of_column;
	...
	the_query := 'SELECT * FROM 
		rc_show_content_of_a_column('||quote_literal(schema_name)||','||quote_literal(table_name)||','||quote_literal(first_column_name)||','||max_number_of_row||') AS column_content(a_serie bigint, '||first_column_name||' varchar) ' ;
	...
	LOOP
		...
		LEFT OUTER JOIN 
			( SELECT * FROM 
			 rc_show_content_of_a_column('||quote_literal(schema_name)||','||quote_literal(table_name)||','||quote_literal(r.column_name)||','||max_number_of_row||') AS column_content(a_serie bigint, '||r.column_name||' varchar) 
			) AS '||r.column_name||'
		USING(a_serie)
		 ';
